...
import { useCreatePortfolio, useUpdatePortfolio, usePortfolios } from '@/hooks/queries/usePortfolios';
...
export default function PortfolioForm({ mode, portfolio }: PortfolioFormProps) {
  ...
  const currentPreviewCount = portfolios.filter((p) => {
    if (mode === 'edit' && portfolio && p.seq === portfolio.seq) return false;
    return p.isPreview;
  }).length;
  ...
  const initializeInfoData = () =>
    FIXED_INFO_FIELDS.map((field) => {
      ...
      return { code: field.code, name: field.name, value: existing?.value || '' };
    });
  ...
  const handleThumbnailFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      const result = await uploadImage({ file, folder: 'thumbnails' });
  ...
  const handleMultipleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (failCount > 0) alert(`${failCount}개의 이미지 업로드에 실패했습니다.`);
    } finally {
      setUploadingCount(0);
      e.target.value = '';
    }
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    ...
    if (mode === 'create') {
      ...
      await updatePortfolio({ seq: portfolio!.seq, data: payload });
    }
  };
